...
import {isMesh, isShapeGeometry} from "three-is";
...
const sync = () => {
  ...
  if (object && isMesh(object) && isShapeGeometry(object.geometry)) {
...
objectAttributeChangeSubject.subscribe((editValue) => {
  ...
  if (name === 'name') {
    ...
    const geometry = object.geometry as THREE.ShapeGeometry;
...
// ui -> three
const threeToUi = (geometry: THREE.ShapeGeometry) => {
  form.type = geometry.type;
  form.uuid = geometry.uuid;
  form.name = geometry.name;
  form.curveSegments = geometry.parameters.curveSegments
}
...
<template>
  <el-form :model="form" label-position="left" label-width="80" size="small">
    ...
    <input-number-item label="曲线分段" name="curveSegments"/>
  </el-form>
</template>
