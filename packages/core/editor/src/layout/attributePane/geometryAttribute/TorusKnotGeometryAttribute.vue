...
const {objectAttributeChangeSubject, toggle} = useGeometryAttributeProvide({
  ...
  getNewGeometry: (geometry, name, value) => {
    return new THREE.TorusKnotGeometry(
        ...
        getGeometryValue(geometry, "p", name, value),
        getGeometryValue(geometry, "q", name, value)
    );
  }
})
...
<template>
  <el-form label-position="left" label-width="80" size="small">
    ...
    <input-number-item :name="['parameters','p']" label="管长弧度"/>
    <input-number-item :name="['parameters','q']" label="扭曲弧度"/>
  </el-form>
</template>
